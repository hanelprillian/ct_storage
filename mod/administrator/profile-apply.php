...
<p>The requested URL $_SERVER[PHP_SELF] was not found on this server.</p>");
	...
	if($_POST['save_general_set']) {
		...
		$q=$db->prepare("UPDATE users SET fullname = :fn, email = :email, address= :address where id = :id and role = '1'");
		...
		if($_FILES['filePhoto']['tmp_name']=="") {
		    ...
		    $allowext=array("jpeg","jpg","gif","png","jpe");
		      ...
		      if($upload) {
		        ...
		        $q=$db->prepare("UPDATE users SET photo = :photo where id = :id and role = '1'");
		...
		header('location:?mod=administrator&page=profile&msg=success&id_msg=1');
		exit();
		
	} else if($_POST['save_password_set']) {
		if($_POST['old_pass']=="" || $_POST['new_pass']=="") {
			header('location:?mod=administrator&page=profile&msg=error&id_msg=2');
			exit();
		} else if($_POST['old_pass']==$_POST['new_pass']) {
			header('location:?mod=administrator&page=profile&msg=error&id_msg=3');
			exit();
		} else {
			...
			$q=$db->prepare("SELECT * FROM users where id = :id and password = :oldpass and role= '1'");
			$q->bindParam(":id",$_SESSION['user_id']);
			...
			$check=$q->rowCount();
			if($check == 1) {
				$q=$db->prepare("UPDATE users set password = :newpass where id = :id and role= '1'");
				$q->bindParam(":newpass",$new_pass);
				$q->bindParam(":id",$_SESSION['user_id']);
				$q->execute();
				header('location:?mod=administrator&page=profile&msg=success&id_msg=2');
				exit();
			} else {
				header('location:?mod=administrator&page=profile&msg=error&id_msg=1');
				exit();
			}
		}
	} else {
		header('location:?mod=administrator&page=profile');
		exit();
	}
?>
