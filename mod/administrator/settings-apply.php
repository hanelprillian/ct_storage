...
<p>The requested URL $_SERVER[PHP_SELF] was not found on this server.</p>");
	...
	if($_POST['save_general_set']) {
		...
		$q=$db->prepare("UPDATE users SET fullname = :fn, email = :email, address= :address where id = :id and role = :role");
		$q->bindParam(":fn",$fullname);
		$q->bindParam(":email",$email);
		$q->bindParam(":address",$address);
		$q->bindParam(":id",$_SESSION['user_id']);
		$q->bindParam(":role",$_SESSION['role']);
		...
		if($_FILES['filePhoto']['tmp_name']=="") {
		    ...
		    $allowext=array("jpeg","jpg","gif","png","jpe");
		      ...
		      if($upload) {
		        ...
		        $q=$db->prepare("UPDATE users SET photo = :photo where id = :id and role = :role");
		...
		if($_POST['old_pass']=="" || $_POST['new_pass']=="") {
			...
			header('location:?mod=administrator&page=settings&msg=error&id_msg=3');
			exit();
		} else {
			$old_pass=trim(htmlentities(htmlspecialchars(addslashes(md5($_POST['old_pass'])))));
			$new_pass=trim(htmlentities(htmlspecialchars(addslashes(md5($_POST['new_pass'])))));
			$q=$db->prepare("SELECT * FROM users where id = :id and password = :oldpass and role= :role");
			$q->bindParam(":id",$_SESSION['user_id']);
			...
			$check=$q->rowCount();
			if($check == 1) {
				$q=$db->prepare("UPDATE users set password = :newpass where id = :id and role= :role ");
				...
				header('location:?mod=administrator&page=settings&msg=error&id_msg=1');
				exit();
			}
		}
	} else {
		header('location:?mod=administrator&page=settings');
		exit();
	}
?>
